...
const App: React.FC = () => {
  ...
  const [activeLegalModal, setActiveLegalModal] = useState<'privacy' | 'tos' | 'disclaimer' | null>(null);
  ...
  return (
    <div className="min-h-screen w-full flex flex-col items-center bg-[#020617] bg-gradient-to-br from-[#020617] via-[#0f172a] to-[#1e1b4b] pb-24 relative overflow-x-hidden">
      ...
      <footer className="mt-24 pb-12 flex flex-col items-center gap-10">
        <nav className="flex flex-wrap justify-center gap-x-8 gap-y-4 text-[10px] font-bold uppercase tracking-widest text-slate-500">
          <button onClick={() => setIsContactOpen(true)} className="hover:text-blue-400 transition-colors">Contact</button>
          <button onClick={() => setActiveLegalModal('privacy')} className="hover:text-blue-400 transition-colors">Privacy Policy</button>
          <button onClick={() => setActiveLegalModal('tos')} className="hover:text-blue-400 transition-colors">Terms</button>
          <button onClick={() => setActiveLegalModal('disclaimer')} className="hover:text-blue-400 transition-colors">Disclaimer</button>
        </nav>
        <div className="flex flex-col items-center gap-2">
          ...
          <p className="text-[9px] text-slate-700 italic font-medium">Verified Publisher & Affiliate Portal</p>
        </div>
      </footer>
        ...
        <div className="fixed bottom-0 left-0 w-full z-[100] p-4 animate-bounce-in">
          <div className="max-w-4xl mx-auto bg-slate-900/90 backdrop-blur-xl border border-white/10 p-6 rounded-3xl shadow-2xl flex flex-col md:flex-row items-center justify-between gap-6">
            <p className="text-xs text-slate-300 text-center md:text-left">
              Kami menggunakan cookie untuk memberikan pengalaman terbaik. Dengan melanjutkan, Anda menyetujui kebijakan cookie kami.
            </p>
            <div className="flex gap-4">
              <button onClick={() => setActiveLegalModal('privacy')} className="text-[10px] font-bold uppercase text-slate-400 hover:text-white">Detail</button>
              <button onClick={handleCookieAccept} className="px-6 py-2 bg-blue-600 hover:bg-blue-500 text-white text-[10px] font-black rounded-xl uppercase tracking-widest">Lanjutkan</button>
            </div>
          </div>
        </div>
      )}
          ...
          <div className="bg-slate-900 border border-white/10 w-full max-w-2xl rounded-[2.5rem] p-8 md:p-12 shadow-2xl max-h-[80vh] overflow-y-auto custom-scrollbar">
            <div className="flex justify-between items-center mb-8 sticky top-0 bg-slate-900 py-2 z-10">
              <h2 className="text-2xl font-black text-white uppercase tracking-tight">
                ...
                {activeLegalModal === 'disclaimer' && 'Disclaimer'}
              </h2>
            ...
            <div className="prose prose-invert text-slate-400 text-sm leading-relaxed space-y-4">
              {activeLegalModal === 'privacy' && (
                <>
                  <p>Kebijakan ini merinci bagaimana kami menangani data Anda sesuai dengan standar perlindungan data global.</p>
                  <h3 className="text-white font-bold">Pengumpulan Data</h3>
                  <p>Kami menggunakan log file dan cookie pihak ketiga (seperti Google AdSense) untuk menganalisis tren dan mengelola situs.</p>
                  <h3 className="text-white font-bold">Cookie DART</h3>
                  <p>Google sebagai vendor pihak ketiga menggunakan cookie untuk menayangkan iklan berdasarkan kunjungan pengguna ke situs kami.</p>
                </>
              )}
              {activeLegalModal === 'tos' && (
                <>
                  <h3 className="text-white font-bold">1. Ketentuan Penggunaan</h3>
                  <p>Situs ini disediakan untuk tujuan informasi saja. Kami tidak bertanggung jawab atas keputusan karir yang diambil berdasarkan informasi di situs ini.</p>
                  <h3 className="text-white font-bold">2. Hak Kekayaan Intelektual</h3>
                  <p>Seluruh logo dan konten di situs ini adalah milik Loker Lampung Official kecuali disebutkan lain.</p>
                </>
              )}
              {activeLegalModal === 'disclaimer' && (
                <>
                  <p>Informasi yang diberikan oleh Loker Lampung Official adalah untuk tujuan informasi umum saja.</p>
                  <p><strong>Kemitraan Afiliasi:</strong> Beberapa link di situs ini adalah link afiliasi. Kami mungkin menerima komisi jika Anda melakukan pembelian melalui link tersebut.</p>
                  <p><strong>Verifikasi Loker:</strong> Meskipun kami melakukan verifikasi awal, pengguna disarankan untuk selalu berhati-hati terhadap segala bentuk permintaan uang dalam proses rekrutmen.</p>
                </>
              )}
            </div>
          </div>
          ...
          <div className="bg-slate-900 border border-white/10 w-full max-w-lg rounded-[2.5rem] p-10 shadow-2xl">
            ...
            {contactFormStatus === 'sent' ? (
              ...
              <form onSubmit={handleContactSubmit} className="space-y-4">
                ...
                <textarea required rows={4} className="w-full p-4 bg-white/5 border border-white/10 rounded-2xl text-white outline-none focus:border-blue-500 resize-none" placeholder="Apa yang bisa kami bantu?"></textarea>
